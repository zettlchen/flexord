...
general purpose optimizer `flexclust::centOptim`, just with NA removal. In our case of purely ordinal data, we could also choose the Median as a centroid:
```{r ordinal_p3}
kcca(risk, k=4, family=kccaExtendedFamily('kGower',
                                          cent=centMedian))
```
This results in kMedians with previous scaling, and non-missing value upweighing. In our `risk` example with
no NAs and equal level lengths for all variables, `flexclust::kccaFamily('kmedians')` would suffice, but there are still many data situations where the `kGower` approach will be preferable.

As a second alternative designed specifically for ordinal data, we implement the GDM2 distance for ordinal
...
flexmix(risk~1, k=3,
        model=FLXMCbetabinom(size=6)) #the hypothetical example where response level 6 was never picked
```
...
by centering around their minimum and dividing by the range. This means that also for `kGower`, the range of the variables will influence the clustering results. Same as for `kGDM2`
