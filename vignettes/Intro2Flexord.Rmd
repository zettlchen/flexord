...
distribution. For this, we plug in the driver we provide into `flexmix`:
...
If in our application we are likely to obtain degenerate solutions, we
can mitigate this risk by setting the $\alpha$ parameter
greater 0. With this, we pretend adding $\alpha$ observations which are equal
to the population mean to each component (=cluster):

```{r nominal_m3}
flexmix(data.matrix(titanic_df) ~ 1, k = 3,
        model=FLXMCregmultinom(r = titanic_ncats, alpha = 1)) #"add" 1 observation per component
```
...
```{r ordinal_m2}
flexmix(risk~1, k=3,
        model=FLXMCregbinom(size=5))
flexmix(risk~1, k=3,
        model=FLXMCbetabinom(size=5, alpha=1))
```
