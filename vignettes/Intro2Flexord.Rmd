...
case we need to pass `k = 3`. We could however specify a value for
`zeta_p` and then omit the parameter `k` (we can't have both parameter
at the same time, therefore `zeta_p` takes precedence if both were
given).
